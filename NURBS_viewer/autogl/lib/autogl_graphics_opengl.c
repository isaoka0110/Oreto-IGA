...
void AutoGL_SetLineWidth2 (float width) {   GLfloat range[2];    SetDefaultPrimitive ();    glGetFloatv(GL_ALIASED_LINE_WIDTH_RANGE, range);   if (width < range[0]) width = range[0];   if (width > range[1]) width = range[1];    glLineWidth(width); }  void AutoGL_SetColor2 
